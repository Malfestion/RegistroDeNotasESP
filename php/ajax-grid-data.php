...
$query=mysqli_query($conn, $sql) or die("ajax-grid-data.php: get all data");
...
if( !empty($requestData['columns'][0]['search']['value']) ){   //id
    ...
    $sql.=" AND nombre LIKE '%".$requestData['columns'][1]['search']['value']."%' ";
} if( !empty($requestData['columns'][2]['search']['value']) ){  //periodo
    $sql.=" AND periodo LIKE '".$requestData['columns'][2]['search']['value']."%' ";
} if( !empty($requestData['columns'][3]['search']['value']) ){  //nivel
    $sql.=" AND nivel LIKE '".$requestData['columns'][3]['search']['value']."%' ";
} if( !empty($requestData['columns'][4]['search']['value']) ){  //grupo
    $sql.=" AND grupo LIKE '".$requestData['columns'][4]['search']['value']."%' ";
} if( !empty($requestData['columns'][5]['search']['value']) ){  //area
    $sql.=" AND area LIKE '".$requestData['columns'][5]['search']['value']."%' ";
} if( !empty($requestData['columns'][6]['search']['value']) ){  //nota
	$sql.=" AND nota LIKE '".$requestData['columns'][6]['search']['value']."%' ";
} if( !empty($requestData['columns'][7]['search']['value']) ){  //profesor
    $sql.=" AND profesor LIKE '%".$requestData['columns'][7]['search']['value']."%' ";
} 
	
	$query=mysqli_query($conn, $sql) or die("ajax-grid-data.php: get searchs");
	...
	$query=mysqli_query($conn, $sql) or die("ajax-grid-data.php: get orders"."sql: ".$sql);
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = '<a href="update_reg_anteriores.php?edit='.$row['edit'].'"  data-toggle="tooltip" title="Editar datos" class="btn btn-sm btn-link">Editar Registro #'.$row['edit'].' </a>';
